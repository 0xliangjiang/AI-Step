    ...
    def _execute_brush_step(self, user_key: str, target_steps: int) -> bool:
        ...
        try:
            with get_db_session() as db:
                ...
                if USE_PROXY_MODE:
                    # 通过国内刷步微服务刷步；传入缓存 token 加速（微服务内部会在 token 过期时自动重登录）
                    result = step_client.brush(
                        ...
                        userid=user.zepp_userid,
                    )
                    # 更新 token 缓存
                    if result.get("login_token") and result.get("app_token"):
                        user.login_token = result["login_token"]
                        user.app_token = result["app_token"]
                        from datetime import datetime
                        user.token_updated_at = datetime.now()
                else:
                    result = step_client.bind(
                        user.zepp_email,
                        user.zepp_password,
                        step=target_steps,
                        use_proxy=False,
